...
func (g *SineWave) Stream(samples [][2]float64) (n int, ok bool) {
	...
	g.nSample -= len(samples)
	return g.nSample, false
}
...
func SineTone(sr beep.SampleRate, freq float64, dur time.Duration) (beep.Streamer, error) {
	dt := freq / float64(sr)
...
func Start() {
	...
	sine, _ := SineTone(sr, 100, time.Second*3)
	done := make(chan bool)
	speaker.Play(beep.Seq(sine, beep.Callback(func() {
		done <- true
	})))
	<-done
}
